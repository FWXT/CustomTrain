...
def inspect_unchange_ratio(input_file):
    stat = {}
...
def balance_data(input_file, output_file, negative_keep_prob: float = 0.333, shuffle: bool = True):
    ...
    before_stat = {
        "all": before_all_num,
        "is_unchanged": before_negative_num,
        "is_unchanged_ratio": is_unchanged_ratio
    }
    # after change
    ...
    after_stat = {
        "all": after_all_num,
        "is_unchanged": after_negative_num,
        "is_unchanged_ratio": is_unchanged_ratio
    }
...
def balance_folder(folder_path, negative_keep_prob=0.25, shuffle=True):
    """Process all JSON files containing 'zeta' in the given folder, balance them and save with 'balanced_' prefix.
